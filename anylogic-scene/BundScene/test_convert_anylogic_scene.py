...
class AnyLogicConversionTests(unittest.TestCase):
    def test_default_grid_resolution_uses_higher_density(self) -> None:
        ...
        self.assertEqual(DEFAULT_CHANNEL_ROUTE_MODE, "pass_through")
    ...
    def test_extracts_anylogic_paths_as_wall_records(self) -> None:
        ...
        self.assertGreaterEqual(path6.raw_point_count, 90)
    ...
    def test_auxiliary_route_regions_are_preserved(self) -> None:
        ...
        self.assertEqual(route_regions["waypoint_channel2_to_channel3"], "route")
    ...
    def test_default_routes_use_channel_split_when_auxiliary_regions_exist(self) -> None:
        ...
        self.assertIn('decision_regions = ["goal_region", "channel_2"]', routes_text)
        self.assertIn('stage_id = "to_pre_channel_3"', routes_text)
        self.assertIn('decision_regions = ["goal_region2", "channel_3"]', routes_text)
        self.assertIn('stage_id = "to_pre_channel_4"', routes_text)
        self.assertIn('decision_regions = ["goal_region5", "channel_4"]', routes_text)
        self.assertIn('stage_id = "to_channel_1"', routes_text)
        self.assertIn('goal_region = "merge_after_channels"', routes_text)
        self.assertIn('decision_regions = ["channel_1", "post_channel_1"]', routes_text)
        self.assertRegex(
            routes_text,
            (
                r'stage_id = "to_channel_2"\n'
                r'group_key = \[1, \d+\]\n'
                r'goal_region = "waypoint_channel2_to_channel3"\n'
                r'decision_region = "waypoint_channel2_to_channel3"\n'
                r'next_stage = "to_merge_after_channels"'
            ),
        )
        self.assertIn('decision_regions = ["channel_4", "post_channel_4"]', routes_text)
        self.assertIn('next_stage = "to_merge_after_channels"', routes_text)
        self.assertNotIn('stage_id = "post_channel_1"', routes_text)
        self.assertNotIn('goal_region = "post_channel_1"', routes_text)
        self.assertNotIn('decision_region = "post_channel_1"', routes_text)
        self.assertIn('next_stage = "to_merge_after_channels"', routes_text)
        self.assertIn('stage_id = "to_channel_4"', routes_text)
        self.assertNotIn('stage_id = "post_channel_4"', routes_text)
        self.assertNotIn('goal_region = "post_channel_4"', routes_text)
        self.assertNotIn('decision_region = "post_channel_4"', routes_text)
        self.assertIn('stage_id = "to_merge_after_channels"', routes_text)
        self.assertIn('decision_regions = ["merge_after_channels", "exits"]', routes_text)
    ...
    def test_direct_merge_route_mode_keeps_channel_as_transition_target(self) -> None:
        _, regions = extract_regions(ALP_PATH, nx=DEFAULT_NX, ny=DEFAULT_NY)
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            write_draft_toml(
                ...
                ny=DEFAULT_NY,
                dx=0.5,
                channel_route_mode="direct_merge",
            )
            routes_text = (output_dir / "routes.toml").read_text(encoding="utf-8")

        self.assertIn('stage_id = "to_channel_1"', routes_text)
        self.assertIn('goal_region = "channel_1"', routes_text)
        self.assertIn('decision_region = "channel_1"', routes_text)
        self.assertIn('next_stage = "to_merge_after_channels"', routes_text)
        self.assertNotIn('stage_id = "post_channel_1"', routes_text)

    def test_legacy_post_stage_route_mode_can_be_restored(self) -> None:
        _, regions = extract_regions(ALP_PATH, nx=DEFAULT_NX, ny=DEFAULT_NY)
        ...
        self.assertIn('decision_region = "post_channel_1"', routes_text)
