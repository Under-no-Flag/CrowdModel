...
3. 建立区分机制验证与策略优化的评价体系，将实验指标划分为行为机制指标（方向一致性、接近角分布、通道捕获域等）和管控优化指标（总旅行时间、高密度暴露时间、实际通道流量方差、入口等待和控制平滑性），避免以系统绩效替代机制验证。

4. 设计面向方向配置与入口容量控制的分层约束混合变量优化框架，在固定历史偏好参数和固定几何引导参数下，联合优化通道方向配置 $s$ 与内部入口通行速率 $q$，并通过多种横向对比算法检验其在有限仿真预算下的效果。
