...
\begin{abstract}
This paper presents a macroscopic crowd management framework for open urban pedestrian zones with multiple channels, staged visitor behavior, and internal entrance flow control. The lower level employs a Hughes-type Bellman--conservation-law model in which one-way, bidirectional, and closed channel states are encoded through admissible direction sets, while a fixed anisotropic metric tensor represents soft geometric channel alignment. The controllable management variable is defined as $z=(s,q)$, where $s$ denotes channel direction configurations and $q$ denotes internal channel-entrance capacity profiles. The geometric guidance strength is treated as a fixed model parameter $\eta_0$, rather than an optimized control. The upper level minimizes a scalarized objective including total travel time, high-density exposure, realized channel-load imbalance, entrance waiting, and control smoothness. A hierarchical constrained mixed-variable black-box optimization framework is developed to search the direction-capacity control space under expensive high-fidelity simulations. Numerical experiments on a four-channel waterfront-platform scenario validate the model mechanisms, the necessity of multi-stage behavior modeling, and the performance of the proposed optimizer in a completed horizontal comparison.
\end{abstract}
...
\item A hierarchical constrained mixed-variable black-box optimization framework is designed for the controllable direction-capacity variable $z=(s,q)$ under fixed historically identified preference parameters and fixed geometric guidance. The framework is evaluated against random search, pure simulated annealing, enumeration plus differential evolution, and TPE-based mixed-variable Bayesian optimization.
\end{enumerate}
...
\end{equation}
The crossing flux at the internal entrance is scaled by $\hat A_c^\pm(t)/A_c^\pm(t)$ when $A_c^\pm(t)>0$. The blocked part remains upstream and is recorded as entrance waiting or queueing mass, preserving the mass balance of the conservation-law update.
...
\textbf{Evaluation Oracle.} For any feasible control configuration $z=(s,q)$, the simulation workflow constitutes a lower-level evaluation operator:
\begin{equation}
...
\subsection{Efficiency, Safety, and Balance}
...
\label{eq:j2}
\end{equation}
Let $R_c(T)=\int_0^T(\hat A_c^+(t)+\hat A_c^-(t))\,dt$ be the realized cumulative throughput of channel $c$. The load-balancing metric is:
\begin{equation}
...
\subsection{Entrance Waiting and Control Smoothness}
...
\label{eq:jb}
\end{equation}
If explicit queues are not stored, the blocked demand $(A_c^\pm-\hat A_c^\pm)_+$ can be used as an equivalent waiting proxy.
...
\begin{equation}
\tilde{J}_1(z) = \frac{J_1(z)}{M_{\text{tot}} T},
\label{eq:j1_norm}
\end{equation}
\begin{equation}
\tilde{J}_2(z) = \frac{J_2(z)}{|\Omega_w| T},
\label{eq:j2_norm}
\end{equation}
and
\begin{equation}
\tilde{J}_5(z) =
\frac{\text{Var}(R_1, \dots, R_C)}{R_{\text{tot}}^2 (C-1)/C^2}
=
\frac{C^2}{C-1}\text{Var}(r_1, \dots, r_C),
\label{eq:j5_norm}
\end{equation}
where $r_c=R_c/R_{\text{tot}}$. Waiting and smoothness terms are normalized by fixed reference scales, giving $\tilde J_B$ and $\tilde J_R$.
...
\begin{equation}
s=(s_1,\ldots,s_C),\qquad s_c\in\{-1,0,+1,\varnothing\},
\end{equation}
...
\subsection{Hierarchical Constrained Mixed-Variable Optimization}
...
\begin{enumerate}
\item direction-candidate generation and filtering;
\item direction-aware capacity parameterization $q=T_s(x)$;
\item constrained surrogate or finite-budget black-box search over $x$;
\item derivative-free local refinement for promising candidates;
\item uniform high-fidelity re-evaluation for final ranking.
\end{enumerate}

\subsection{Direction Candidate Filtering}

The outer level generates a set $\mathcal S_{\mathrm{cand}}$ of direction configurations and removes candidates that violate operational constraints, connectivity requirements, or the direction-capacity consistency in \eqref{eq:direction_capacity_consistency}. The retained set provides interpretable discrete alternatives for the inner capacity optimization.

\subsection{Capacity Search and High-Fidelity Recheck}

For a fixed direction configuration $s$, the inner level optimizes $x$ in $q=T_s(x)$. The search may use Bayesian optimization, evolutionary search, or another finite-budget derivative-free method, but all candidates are passed through the same feasibility-preserving map $T_s$. Promising candidates can be refined locally, after which all finalists are re-evaluated under the same high-fidelity simulator. The paper reports only high-fidelity recheck results.
...
Numerical parameters are set as follows: grid resolution $128\times96$, spatial step $\Delta x=0.5$, free-flow speed $v_{\max}=1.5$, maximum density $\rho_{\max}=5.0$, initial density $\rho_{\text{init}}=2.2$, CFL coefficient $0.9$, and safety density threshold $\rho_{\text{safe}}=3.5$. The capacity-control horizontal comparison uses $1600$ high-fidelity steps, a time horizon of $T=160$, and Bellman updates every five simulation steps.
...
The completed horizontal comparison evaluates baseline prior best, Random Search, Pure SA, Enum-DE, TPE-Mixed BO, and HCMBO under the same high-fidelity setting. The result root is \texttt{codes/results/g6\_horizontal\_comparison}. Five seeds are used: $11$, $23$, $37$, $51$, and $73$.
...
Table~\ref{tab:g6_summary} shows that TPE-Mixed BO obtains the lowest mean objective and the best single-seed objective among all reported methods. HCMBO outperforms Random Search, Pure SA, and Enum-DE in mean objective, and shares the highest feasible rate of $80\%$ with TPE-Mixed BO. The current paper figure set excludes TPE-Mixed BO to focus on interpretable structured baselines; under this retained-method subset, HCMBO has the lowest mean objective and the highest feasible rate.

\section{Conclusion}
\label{sec:conclusion}

This paper presents a Bellman--conservation-law crowd management framework for open pedestrian zones with channel direction configuration and internal entrance capacity control. The current controllable variable is $z=(s,q)$; the geometric guidance parameter $\eta_0$ and route preference parameter $\hat p$ are fixed model inputs. The proposed HCMBO framework exploits the direction-capacity hierarchy and uses high-fidelity rechecks for final ranking.

The numerical experiments validate that: (1) fixed geometric guidance $M(x;\eta_0)$ and directional constraints $U(x;s)$ alter local crowd behaviors in interpretable ways; (2) directional configurations induce nontrivial efficiency--safety--balance trade-offs; (3) multi-stage behavior modeling is necessary for reliable channel-load assessment; and (4) in the completed horizontal comparison, HCMBO is the best retained structured method when TPE is excluded from the figure set, while TPE-Mixed BO obtains the lowest mean objective in the full method table.

Future work will extend to larger real-world geometries, online demand estimation, multi-weight Pareto analysis, and calibration against field observations.
