...
Under a dimensionless composite objective obtained by weighting normalized travel-time, high-density-exposure, channel-load-imbalance, waiting, and control-smoothness terms, repeated experiments show that HCMBO achieves the lowest mean objective value.
...
Thus, $\bar J_1$ is a relative system-residence measure, $\bar J_2$ is the average hard exposure or soft safety-risk intensity over the walkable space-time domain, and $\bar J_3$ is zero for perfectly balanced realized channel use and approaches one when all realized channel throughput concentrates on one channel.
...
Although $J$, $J^{(3)}$, and $\tilde J_k$ are dimensionless ranking quantities, their interpretation is anchored by the normalization above.
When physical interpretation is needed, the experiments also report raw or ratio-based diagnostics such as channel-flow shares, unreleased attempted mass, binding-time ratios, system mass, and density peaks.

For mechanism-verification experiments and fixed-strategy diagnostic experiments that only test direction rules, geometric guidance, or fixed entrance-rate responses, this paper uses the reduced three-term objective
\begin{equation}
J^{(3)}(z)=
\lambda_1\tilde J_1(z)
+\lambda_2\tilde J_2(z)
...
For these reduced-objective experiments, $\tilde J_4$ is only an entrance-waiting diagnostic or is not applicable, and $\tilde J_5$ is not applicable; equivalently, $\lambda_4=\lambda_5=0$.
Therefore, the numerical values of $J$ and $J^{(3)}$ should not be compared across experiment groups; comparisons are meaningful only within the same objective definition, normalization rule, and weight setting.
...
Mass-time quantities are reported in simulation mass-time units, and counterflow cell-time is reported in grid-cell-time units.
...
The controlled group has a higher efficiency term ($\tilde J_1$ increases from $0.3813$ to $0.4145$), a slightly higher safety-exposure term ($\tilde J_2$ increases from $0.0284$ to $0.0357$), a smaller cumulative sink flow ($6.82$ versus $8.57$ simulation mass units), and a larger final system mass ($7.85$ versus $6.18$ simulation mass units).
...
Under five random seeds, $B=400$ candidate evaluations, and unified high-fidelity rechecks, HCMBO obtains the lowest mean dimensionless full objective, reducing the fixed-weight management score by $3.9\%$ relative to TPE-Mixed BO while maintaining the highest feasible rate of $100\%$.
This improvement should not be interpreted as an isolated reduction in travel time, density exposure, or waiting alone.
