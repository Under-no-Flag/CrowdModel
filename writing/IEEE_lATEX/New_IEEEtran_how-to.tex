...
\title{Bellman Conservation Law-based Crowd Flow Control via Passage Direction and Entrance-Rate Optimization in Metropolitan Tourism Hotspots With Capacity of 50,000 Pedestrians}
...
Scaling from the demonstration level to a 50,000-pedestrian operating condition therefore requires no change to the model structure, the numerical scheme, or the grid; only demand-side parameters change.
Table~\ref{tab:bund_scale_projection} contrasts the two operating conditions.
...
\begin{table*}[!t]
\centering
\caption{Parameter Correspondence Between the Transfer Experiment and a Real-Scale Bund Operating Condition}
...
\hline
Parameter & Transfer experiment & Real-scale condition & Effect on cost \\
\hline
...
The real-world transfer experiment inspired by the Shanghai Bund scenario further demonstrates the strong practical generalization capability of the proposed framework. Derived purely from the HCMBO optimizer, the optimized crowd management policy reorganizes pedestrian flow in a high-fidelity spatial abstraction of the hotspot, reducing the overall management objective by 26.9\% by dynamically steering excess crowds away from the severely overloaded terminal channel and rebalancing pedestrian load across the entire middle-channel group. Collectively, these quantitative results establish the proposed framework as an effective, interpretable, and deployable foundation for proactive, optimization-driven crowd management in complex public pedestrian spaces.
