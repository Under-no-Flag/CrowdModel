...
```bash
python .claude/skills/tikz/scripts/render_tikz.py <path-to-tex-file>
```
...
adjust font size, reposition elements, etc.), then re-render:
  python .claude/skills/tikz/scripts/render_tikz.py <tex-path>
...
python .claude/skills/tikz/scripts/submit_temp.py <path-to-file>

# Symlink instead of copy (good for large files or live-editing)
python .claude/skills/tikz/scripts/submit_temp.py --link <path>

# Symlink a directory
python .claude/skills/tikz/scripts/submit_temp.py --link /path/to/docs/

# Custom name
python .claude/skills/tikz/scripts/submit_temp.py --name translated.md <path>
```
...
```bash
python .claude/skills/tikz/scripts/ensure_server.py --stop
```
