...
- 比例缩放根据每个群体或网格面的原始尝试流量占总尝试流量的比例来分配实际可通过量。


公式上：
$$\widehat F_{g,f} = \frac{F_{g,f}^{\mathrm{free}}}{\sum_{g,f} F_{g,f}^{\mathrm{free}}} \cdot q_c^\pm$$
这里 $F_{g,f}^{\mathrm{free}}$ 是自由尝试流量，$\widehat F_{g,f}$ 是实际分配的受限流量。
...
- 所有分配后的通量之和正好等于通道允许的最大通量 $q_c^\pm(t)$。
...
- 各子群体的通行机会按照它们的需求比例分配，而不是随机或单独限制。


- 在多群体情况下，有助于模拟现实中“竞争有限通道的分流效应”。
---
