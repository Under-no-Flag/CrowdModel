...
1. **通道方向配置**
   $$
   s=(s_1,\dots,s_C),\qquad s_c\in\{-1,0,+1,\varnothing\},
   $$
   其中 $s_c=+1$、$-1$、$0$、$\varnothing$ 分别表示第 $c$ 条通道按参考切向正向单向、反向单向、保持双向或完全关闭。关闭状态在下层 Bellman 求解中实现为通道区域允许方向集合为空，即 $U_c(x)=\varnothing$，数值上对应该区域不可达或所有从该区域出发的步进代价为无穷大。
...
2. 下层模型采用“共享拥堵、分离势场”的多子群体结构，单次全仿真代价高；
3. 离散通道配置与连续引导强度对目标函数的影响不同，需要分块处理。
...
1. **离散块邻域搜索**：对含关闭状态的通道方向配置 $s$ 进行局部搜索，而非全枚举；
2. **代理预筛选**：对邻域候选先做局部代理评价，仅将少量候选送入全仿真；
3. **全量真实评估确认**：对代理保留候选运行完整下层仿真，以真实目标值作为接受和排序依据；
4. **连续块投影随机近似更新**：对 $\eta$ 做扰动搜索，并加入接受/回退机制保证流程闭合。

因此，本文不将 SA-HBO 表述为通用元启发式算法，而将其表述为：
**利用控制变量分块结构、代理预筛选和真实仿真评估而构造的结构感知优化框架。**
...
考虑到通道方向变量 $s_c$ 仅会改变对应通道区域内的允许方向集合 $U(x)$，从而局部影响 Bellman 步进结构，而不会改变整个区域的数学形式，本文采用邻域搜索而非全局枚举。对于每条通道，候选状态包括正向单向、反向单向、双向和关闭四类；若通道数为 $C$，全枚举规模为 $4^C$，因此更需要利用局部邻域搜索。定义当前方向配置 $s^k$ 的邻域为
$$
\mathcal N(s^k)
=
\left\{
...
2. 对 $k=0,1,\dots,K_{\max}-1$ 重复：
   ...
   **离散块更新**
   ...
   4. 对 $s\in\mathcal C_K(s^k)$ 执行完整下层仿真，计算真实目标值 $J(s,\eta^k\mid \hat p)$；
   5. 更新
      $$
...
本文算法中，离散块每次只对邻域候选做廉价代理评估，再对少量候选做完整真实评估。因此，单次离散块更新的复杂度约为
$$
O\!\left(
...
\right),
$$
其中 $C_{\mathrm{proxy}}$ 为代理评价成本，$K$ 为代理保留并进入完整仿真的候选数。

连续块更新中，当前论文只对 $\eta$ 做一次双侧扰动，因此其主成本约为
$$
O(2\,C_{\mathrm{full}}),
$$
而不随 $\eta$ 维数线性增长。因此，精简版 SA-HBO 的主要节省来自代理预筛选减少真实仿真候选数，以及连续块双侧扰动避免按维度逐一差分。

---

## 9. 方法创新点表述

本文的方法创新不在于调用某一通用元启发式方法，而在于：针对现有 Bellman–守恒律耦合人群模型的控制变量结构，提出了一个**结构感知的混合优化框架**，并在当前论文中实现了其针对 $(s,\eta)$ 的精简实例。具体体现在以下四个方面。

### 9.1 可控变量与行为参数的清晰分离
本文明确将通道方向配置 $s$（包括单向、双向和关闭）和几何引导强度 $\eta$ 视为可控管控变量，而将游客路线偏好参数 $p=\hat p$ 视为由历史数据识别得到的外生行为参数，不将其直接纳入优化。这使得优化问题与实际管理权限保持一致。
