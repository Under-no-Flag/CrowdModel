...
> 本文件用于替代 `optimization.md` 中以 $z=(s,\eta)$ 为优化变量的表述。V2 版本中，几何引导强度 $\eta$ 不再作为优化变量，而固定为 $\eta_0$；优化变量改为通道方向配置 $s$ 与内部通道入口通行速率 $q$。因此，优化问题从“方向配置 + 几何引导强度搜索”转为“方向配置 + 入口容量控制搜索”。
